...
class Data extends AbstractHelper
{
	...
	public function toShowingServiceArray()
    {	
	...
	public function getQasByTopicIdLimited($topicId)
	{
		...
		$collection = $this->_qaCollectionFactory->create();
		$collection->addFieldToFilter('topic_id',array('eq' => $topicId));
		$collection->setPageSize(10000)->setCurPage(1);
		foreach($collection as $qa){
			$qas[$qa->getQaId()]['question']=$qa->getQuestion();
			$qas[$qa->getQaId()]['answer_summary']=$qa->getAnswerSummary();
			$qas[$qa->getQaId()]['url']=$qa->getUrl();
			$qas[$qa->getQaId()]['showing_service']=$qa->getShowingService();
		}
		return $qas;
	}
	...
	public function getSearchResults($q) {
	    ...
	    $collectioncms = $objectManager->get('\Magento\Cms\Model\ResourceModel\Page\CollectionFactory')->create();

	    $collectioncms->addFieldToFilter(
	    ...
	    foreach($collectioncms as $key=>$page){
		    if($page->getSearchFaqs() == 1) {
				$pages[] = array(
				    'id' => $page->getPageId(),
				    'title' => $page->getTitle(),
