 ...
 */
namespace Stalwart\Productmanager\Controller;
...
class Router implements \Magento\Framework\App\RouterInterface
{
    ...
    public function match(\Magento\Framework\App\RequestInterface $request)
    {
        ...
        $isProductUrl = false;
        if (strpos($request->getPathInfo(), 'en_us/product/') !== false)
        {
            $isProductUrl = true;
        }

        $replaceUrl = str_replace("en_us/product/", "", $request->getPathInfo());
        if($isProductUrl)
            $rewrite = $this->getRewrite($replaceUrl, $this->storeManager->getStore()->getId(),'product');
        else
        ...
        if ($rewrite === null && false) 
        {
            $pathInfo = str_replace("en_us/product/", "", $request->getPathInfo());
            ...
            elseif((isset($pathInfoArray[count($pathInfoArray) - 2])) && (!empty(trim($pathInfoArray[count($pathInfoArray) - 2]))))
            ...
            if($key != "")
            {
                ...
                if($collection->count())
                {
                    ...
                    $key = '/' . $key;
                    if($isProductUrl)
                        $rewrite = $this->getRewrite($key, $this->storeManager->getStore()->getId(),'product');
                    else
     ...
     */
    protected function getRewrite($requestPath, $storeId,$entityType = '')
    {
        if($entityType)
        {
