...
class Order extends AbstractModel implements OrderInterface
{
    ...
    public function __construct(
        ...
        \Stalwart\Sweda\Model\ResourceModel\Invoice\CollectionFactory $invoicefactory,
        array $data = []
    )
    {
    ...
    {
        return $this->invoicefactory->create()->addFieldToFilter('order_number',$this->getOrderNumber());
    }
    public function getShipmentTrackingUrls()
    {
        $urls = array();
        $shipmentArray = array();

        $orderInvoices = $this->getInvoices();
        if($orderInvoices)
        {
            foreach($orderInvoices as $orderInvoice)
            {
                $shippingCompany = $orderInvoice->getShipVia();
                $trackingNumber = $orderInvoice->getWaybillNumber();
                if($shippingCompany && $trackingNumber)
                {
                    $shipmentArray[$shippingCompany][$trackingNumber] = $trackingNumber;
                }
            }
        }
        ...
        if($shipmentArray)
        {
            foreach($shipmentArray as $company => $trackingNums)
            {
                if(strtolower($company) == "ups")
                    $urls[] = "https://wwwapps.ups.com/WebTracking/track?track=yes&trackNums=".implode(",", $trackingNums);
                elseif(strtolower($company) == "fedex")
                    $urls[] = "https://www.fedex.com/apps/fedextrack/index.html?tracknumbers=".implode(",", $trackingNums);
                /*else
                    $urls[] = "https://www.example-".$company.".com/?tracknumbers=".implode(",", $trackingNums);*/
            }
        }
        return $urls;
    }
