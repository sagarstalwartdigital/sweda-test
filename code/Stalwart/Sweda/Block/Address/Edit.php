 ...
 */
namespace Stalwart\Sweda\Block\Address;
...
class Edit extends \Magento\Directory\Block\Data
{
    ...
    {   
        ...
        if ($this->getRequest()->getParam('id') && !empty($this->getRequest()->getParam('id'))) {
            $currentCustomerAddressId = $this->getRequest()->getParam('id');
        } else {
            $objectManager =  \Magento\Framework\App\ObjectManager::getInstance();
            $currentCustomerId = $this->_customerSession->getCustomerId();
            
            $customerFactory = $objectManager->get('\Magento\Customer\Model\CustomerFactory')->create()->load($currentCustomerId);
            $billingAddressId = $customerFactory->getDefaultBilling();
            $currentAddressId = $objectManager->get('Magento\Customer\Model\AddressFactory')->create()->load($billingAddressId);

            $currentCustomerAddressId = $currentAddressId->getData('entity_id');
        }

        // Init address object
        if ($addressId = $currentCustomerAddressId) {
            try {
        ...
        $this->pageConfig->getTitle()->set(__('Edit Your Profile'));
