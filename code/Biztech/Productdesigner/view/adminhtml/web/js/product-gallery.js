...
define([
    ...
    $.widget('mage.productGallery', $.mage.productGallery, {
        ...
        _initDialog: function () {
            ...
            var _changeProductTemplate = function (e) {
                var target = jQuery(e.target);
                var targetName = target.attr('name');
                var desc = target.val();
                var imageData = $dialog.data('imageData');
                jQuery('input[type="hidden"][name="' + targetName + '"]').val(desc);
                imageData.product_template = desc;
                imageData.product_template_default = desc;
            };
            ...
            $dialog.on('change', '#product-template', _changeProductTemplate);
        ...
        onDialogOpen: function (event) {            
            ...
            $(event.target)
                    .find('[data-role=type-productTemplate]')
                    .each($.proxy(function (index, checkbox) {
                        var isChecked = checkbox.value == imageData.product_template_default;
                        var targetName = checkbox.parentElement.getAttribute('name');
                        $('select[name="' + targetName + '"] option[value=' + checkbox.value + ']').prop(
                                'selected',
                                isChecked
