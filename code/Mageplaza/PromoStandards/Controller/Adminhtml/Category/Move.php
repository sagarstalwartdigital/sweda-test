...
class Move extends Category
{
    ...
    {
        /** New parent PromoStandards category identifier */
        $parentNodeId = $this->getRequest()->getPost('pid', false);

        /** PromoStandards category id after which we have put our PromoStandards category */
        ...
        try {
            ...
            if ($category === false) {
                throw new LocalizedException(__('PromoStandards category is not available.'));
            }
            $category->move($parentNodeId, $prevNodeId);
        } catch (LocalizedException $e) {
            $error = true;
            $this->messageManager->addErrorMessage(__('There was a PromoStandards category move error.'));
        } catch (\Exception $e) {
            $error = true;
            $this->messageManager->addErrorMessage(__('There was a PromoStandards category move error.'));
        ...
        if (!$error) {
            $this->messageManager->addSuccessMessage(__('You moved the PromoStandards category'));
        }
