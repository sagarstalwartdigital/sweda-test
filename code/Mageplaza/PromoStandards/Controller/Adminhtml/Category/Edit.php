...
class Edit extends Category
{
    ...
    /**
     * Edit PromoStandards category page
     *
    ...
    public function execute()
    {
        ...
        /**
         * Check if we have data in session (if during PromoStandards category save was exception)
         */
        $data = $this->_getSession()->getData('mageplaza_promostandards_category_data', true);
        ...
        if ($this->getRequest()->getQuery('isAjax')) {
            // prepare breadcrumbs of selected PromoStandards category, if any
            $breadcrumbsPath = $category->getPath();
            if (empty($breadcrumbsPath)) {
                // but if no PromoStandards category, and it is deleted - prepare breadcrumbs from path, saved in session
                $breadcrumbsPath = $this->_objectManager->get('Magento\Backend\Model\Auth\Session')
                    ->getDeletedPath(true);
                if (!empty($breadcrumbsPath)) {
                    $breadcrumbsPath = explode('/', $breadcrumbsPath);
                    // no need to get parent breadcrumbs if deleting PromoStandards category level 1
            ...
            $content = $layout->getBlock('mageplaza.promostandards.category.edit')->getFormHtml()
                       . $layout->getBlock('mageplaza.promostandards.category.tree')
            ...
            $this->_eventManager->dispatch(
                'mageplaza_promostandards_category_prepare_ajax_response',
        ...
        $resultPage->setActiveMenu('Mageplaza_PromoStandards::category');
