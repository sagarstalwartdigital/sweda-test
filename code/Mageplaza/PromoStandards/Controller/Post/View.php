...
namespace Mageplaza\PromoStandards\Controller\Post;
...
use Mageplaza\PromoStandards\Model\TrafficFactory;

/**
 * Class View
 * @package Mageplaza\PromoStandards\Controller\Post
 */
class View extends Action
{
     ...
     */
    protected $helperPromoStandards;
    ...
    public function execute()
    {
        ...
        $post = $this->helperPromoStandards->getFactoryByType(Data::TYPE_POST)->create()->load($id);

        $page = $this->resultPageFactory->create();
        $pageLayout = ($post->getLayout() === 'empty') ? $this->helperPromoStandards->getSidebarLayout() : $post->getLayout();
        $page->getConfig()->setPageLayout($pageLayout);

        if (!$post->getEnabled() || !$this->helperPromoStandards->checkStore($post)) {
            ...
            if (isset($params['cmt_text'])) {
                ...
                if ($this->session->isLoggedIn()) {
                    $commentData = [
                        ...
                        'status'     => $this->helperPromoStandards->getPromoStandardsConfig('comment/need_approve')
