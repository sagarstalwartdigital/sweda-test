...
namespace Mageplaza\PromoStandards\Helper;
...
use Mageplaza\PromoStandards\Model\Author;
use Mageplaza\PromoStandards\Model\AuthorFactory;
use Mageplaza\PromoStandards\Model\Category;
use Mageplaza\PromoStandards\Model\CategoryFactory;
use Mageplaza\PromoStandards\Model\Config\Source\SideBarLR;
use Mageplaza\PromoStandards\Model\Post;
use Mageplaza\PromoStandards\Model\PostFactory;
use Mageplaza\PromoStandards\Model\ResourceModel\Author\Collection as AuthorCollection;
use Mageplaza\PromoStandards\Model\ResourceModel\Category\Collection as CategoryCollection;
use Mageplaza\PromoStandards\Model\ResourceModel\Post\Collection as PostCollection;
use Mageplaza\PromoStandards\Model\ResourceModel\Tag\Collection as TagCollection;
use Mageplaza\PromoStandards\Model\ResourceModel\Topic\Collection;
use Mageplaza\PromoStandards\Model\Tag;
use Mageplaza\PromoStandards\Model\TagFactory;
use Mageplaza\PromoStandards\Model\Topic;
use Mageplaza\PromoStandards\Model\TopicFactory;
use Mageplaza\Core\Helper\AbstractData as CoreHelper;

/**
 * Class Data
 * @package Mageplaza\PromoStandards\Helper
 */
class Data extends CoreHelper
{
    const CONFIG_MODULE_PATH = 'promostandards';
     ...
     * @return mixed
     */
    public function getPromoStandardsConfig($code, $storeId = null)
    {
        $code = ($code !== '') ? '/' . $code : '';
    ...
    {
        return $this->getPromoStandardsConfig('seo/' . $code, $storeId);
    }
     ...
     * @return string
     */
    public function getPromoStandardsName($store = null)
    {
        return $this->getConfigGeneral('name', $store) ?: __('PromoStandards');
    }
    ...
    {
        return $this->getConfigGeneral('url_prefix', $store) ?: 'promostandards';
    }
    ...
    public function getPostCollection($type = null, $id = null, $storeId = null)
    {
        ...
        switch ($type) {
            ...
            case self::TYPE_CATEGORY:
                $collection->join(
                    ['category' => $collection->getTable('mageplaza_promostandards_post_category')],
            ...
            case self::TYPE_TAG:
                $collection->join(
                    ['tag' => $collection->getTable('mageplaza_promostandards_post_tag')],
            ...
            case self::TYPE_TOPIC:
                $collection->join(
                    ['topic' => $collection->getTable('mageplaza_promostandards_post_topic')],
     ...
     * @return string
     */
    public function getPromoStandardsUrl($urlKey = null, $type = null, $store = null)
    {
        if (is_object($urlKey)) {
    ...
    public function getDateFormat($date, $monthly = false)
    {
        ...
        $dateType = $this->getPromoStandardsConfig($monthly ? 'monthly_archive/date_type_monthly' : 'general/date_type');
