...
namespace Mageplaza\Vlibrary\Controller\Post;
...
use Mageplaza\Vlibrary\Model\TrafficFactory;

/**
 * Class View
 * @package Mageplaza\Vlibrary\Controller\Post
 */
class View extends Action
{
    ...
    public function execute()
    {
        ...
        $post = $this->helperVlibrary->getFactoryByType(Data::TYPE_POST)->create()->load($id);

        $page = $this->resultPageFactory->create();
        $pageLayout = ($post->getLayout() === 'empty') ? $this->helperVlibrary->getSidebarLayout() : $post->getLayout();
        $page->getConfig()->setPageLayout($pageLayout);

        if (!$post->getEnabled() || !$this->helperVlibrary->checkStore($post)) {
            ...
            if (isset($params['cmt_text'])) {
                ...
                if ($this->session->isLoggedIn()) {
                    $commentData = [
                        ...
                        'status'     => $this->helperVlibrary->getVlibraryConfig('comment/need_approve')
