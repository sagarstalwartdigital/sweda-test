...
class Save extends Category
{
    ...
    public function execute()
    {
        ...
        if ($data = $this->getRequest()->getPost('category')) {
            $category = $this->initCategory();
            if (!$category) {
                $resultRedirect->setPath('mageplaza_vlibrary/*/', ['_current' => true]);
            ...
            $this->_eventManager->dispatch(
                'mageplaza_vlibrary_category_prepare_save',
                ['category' => $category, 'request' => $this->getRequest()]
            );

            try {
                $category->save();
                $this->messageManager->addSuccess(__('You saved the Vlibrary Category.'));
                $this->_getSession()->setData('mageplaza_vlibrary_category_data', false);
            } catch (\Exception $e) {
                $this->messageManager->addError($e->getMessage());
                $this->_getSession()->setData('mageplaza_vlibrary_category_data', $data);
            }

            $resultRedirect->setPath('mageplaza_vlibrary/*/edit', ['_current' => true, 'id' => $category->getId()]);
