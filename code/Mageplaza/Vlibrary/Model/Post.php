...
use Mageplaza\Vlibrary\Model\ResourceModel\Category\CollectionFactory as CategoryCollectionFactory;
use Mageplaza\Vlibrary\Model\ResourceModel\Post as PostResource;
use Mageplaza\Vlibrary\Model\ResourceModel\Post\Collection;
use Mageplaza\Vlibrary\Model\ResourceModel\Post\CollectionFactory as PostCollectionFactory;
use Mageplaza\Vlibrary\Model\ResourceModel\Tag\CollectionFactory;
use Mageplaza\Vlibrary\Model\ResourceModel\Topic\CollectionFactory as TopicCollectionFactory;
...
class Post extends AbstractModel
{
     ...
     * @var string
     */
    const CACHE_TAG = 'mageplaza_vlibrary_post';
     ...
     * @var string
     */
    protected $_cacheTag = 'mageplaza_vlibrary_post';
     ...
     * @var string
     */
    protected $_eventPrefix = 'mageplaza_vlibrary_post';
    ...
    /**
     * Vlibrary Category Collection Factory
     *
    ...
    {
        return $this->helperData->getVlibraryUrl($this, Data::TYPE_POST, $store);
    }
    ...
    public function getSelectedTagsCollection()
    {
        if ($this->tagCollection === null) {
            ...
            $collection->getSelect()->join(
                $this->getResource()->getTable('mageplaza_vlibrary_post_tag'),
                'main_table.tag_id=' . $this->getResource()->getTable('mageplaza_vlibrary_post_tag') . '.tag_id AND '
                . $this->getResource()->getTable('mageplaza_vlibrary_post_tag') . '.post_id=' . $this->getId(),
                ['position']
    ...
    public function getSelectedTopicsCollection()
    {
        if ($this->topicCollection === null) {
            ...
            $collection->join(
                $this->getResource()->getTable('mageplaza_vlibrary_post_topic'),
                'main_table.topic_id=' . $this->getResource()->getTable('mageplaza_vlibrary_post_topic') . '.topic_id AND '
                . $this->getResource()->getTable('mageplaza_vlibrary_post_topic') . '.post_id=' . $this->getId(),
                ['position']
            );
    ...
    public function getSelectedCategoriesCollection()
    {
        if ($this->categoryCollection === null) {
            ...
            $collection->join(
                $this->getResource()->getTable('mageplaza_vlibrary_post_category'),
                'main_table.category_id=' . $this->getResource()->getTable('mageplaza_vlibrary_post_category') .
                '.category_id AND ' . $this->getResource()->getTable('mageplaza_vlibrary_post_category') . '.post_id="'
                . $this->getId() . '"',
                ['position']
            );
    ...
    public function getRelatedPostsCollection($limit = null)
    {
        ...
        if (count($topicIds)) {
            ...
            $collection->getSelect()
                ->join(
                    ['topic' => $this->getResource()->getTable('mageplaza_vlibrary_post_topic')],
                    'main_table.post_id=topic.post_id AND topic.post_id != "' . $this->getId()
                    . '" AND topic.topic_id IN (' . implode(',', $topicIds) . ')',
                    ['position']
                )->group('main_table.post_id');

            if ($limit = (int) $this->helperData->getVlibraryConfig('general/related_post')) {
                $collection->getSelect()
                    ->limit($limit);
            }
    ...
    public function getSelectedProductsCollection()
    {
        if ($this->productCollection === null) {
            ...
            $collection->getSelect()->join(
                $this->getResource()->getTable('mageplaza_vlibrary_post_product'),
                'main_table.entity_id=' . $this->getResource()->getTable('mageplaza_vlibrary_post_product')
                . '.entity_id AND ' . $this->getResource()->getTable('mageplaza_vlibrary_post_product') . '.post_id='
                . $this->getId(),
                ['position']
            )->where("main_table.enabled='1'");
