...
namespace Mageplaza\Vlibrary\Model\Import;

use Mageplaza\Vlibrary\Model\Config\Source\Comments\Status;

/**
 * Class MageFanM2
 * @package Mageplaza\Vlibrary\Model\Import
 */
class MageFanM2 extends AbstractImport
{
     ...
     * @var string
     */
    const TABLE_POST = 'magefan_vlibrary_post';
    /**
     * Magefan Related Post table name
     *
     * @var string
     */
    const TABLE_POST_RELATED = 'magefan_vlibrary_post_relatedpost';
    /**
     * Magefan Tag table name
     *
     * @var string
     */
    const TABLE_TAG = 'magefan_vlibrary_tag';
    /**
     * Magefan Post-Tag Relationship table name
     *
     * @var string
     */
    const TABLE_POST_TAG = 'magefan_vlibrary_post_tag';
    /**
     * Magefan Category table name
     *
     * @var string
     */
    const TABLE_CATEGORY = 'magefan_vlibrary_category';
    /**
     * Magefan Post-Category Relationship table name
     *
     * @var string
     */
    const TABLE_POST_CATEGORY = 'magefan_vlibrary_post_category';
    /**
     * Magefan Comment table name
     *
     * @var string
     */
    const TABLE_COMMENT = 'magefan_vlibrary_comment';
    /**
    ...
    protected function _importPosts($data, $connection)
    {
        ...
        if ($result) {
            ...
            if ($isReplace && isset($sourceItems)) {
                 ...
                 */
                $topicPostTable = $this->_resourceConnection->getTableName('mageplaza_vlibrary_post_topic');
    ...
    protected function _importTags($data, $connection)
    {
        ...
        if ($isReplace && isset($sourceItems)) {
            ...
            $tagPostTable = $this->_resourceConnection->getTableName('mageplaza_vlibrary_post_tag');
    ...
    protected function _importCategories($data, $connection)
    {
        ...
        if ($isReplace && isset($sourceItems)) {
             ...
             */
            $categoryPostTable = $this->_resourceConnection->getTableName('mageplaza_vlibrary_post_category');
                ...
                if ($comment['is_imported']) {
                    ...
                    $this->_resourceConnection->getConnection()
                        ->update($this->_resourceConnection
                            ->getTableName('mageplaza_vlibrary_comment'), [
    ...
    protected function _importAuthors($data, $connection)
    {
        ...
        foreach ($updateData as $postId => $authorId) {
            ...
            $this->_resourceConnection->getConnection()
                ->update(
                    $this->_resourceConnection->getTableName('mageplaza_vlibrary_post'),
                    ['author_id' => $authorId],
                    $where
                );
        }
    ...
    private function _updatePosts($post, $where)
    {
        $this->_resourceConnection->getConnection()
            ->update($this->_resourceConnection
                ->getTableName('mageplaza_vlibrary_post'), [
                'name'              => $post['name'],
                'short_description' => $post['short_description'],
                'post_content'      => $post['post_content'],
                'url_key'           => $post['url_key'],
        ...
        $this->_resourceConnection->getConnection()
            ->delete($this->_resourceConnection
                ->getTableName('mageplaza_vlibrary_post_category'), $where);
        $this->_resourceConnection->getConnection()
            ->delete($this->_resourceConnection
                ->getTableName('mageplaza_vlibrary_post_tag'), $where);
    }
    ...
    private function _updateTags($tag, $where)
    {
        $this->_resourceConnection->getConnection()
            ->update($this->_resourceConnection
                ->getTableName('mageplaza_vlibrary_tag'), [
                'name'             => $tag['name'],
                'url_key'          => $tag['url_key'],
                'meta_robots'      => $tag['meta_robots'],
                'meta_description' => $tag['meta_description'],
    ...
    private function _updateCategories($category, $where)
    {
        $this->_resourceConnection->getConnection()
            ->update($this->_resourceConnection
                ->getTableName('mageplaza_vlibrary_category'), [
                'name'             => $category['name'],
                'url_key'          => $category['url_key'],
                'meta_robots'      => $category['meta_robots'],
                'store_ids'        => $category['store_ids'],
