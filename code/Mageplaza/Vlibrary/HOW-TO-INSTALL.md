# How to install Vlibrary for Magento 2
...
- Download [the latest version here](https://github.com/mageplaza/magento-2-vlibrary/archive/master.zip) 
- Extract `master.zip` file to `app/code/Mageplaza/Vlibrary`; You should create a folder path `app/code/Mageplaza/Vlibrary` if not exist.
- Go to Magento root folder and run upgrade command line to install `Mageplaza_Vlibrary`:
...
```
Fatal error: Uncaught Error: Call to undefined method Mageplaza\\Vlibrary\\Helper\\Data::isEnabled()
```
