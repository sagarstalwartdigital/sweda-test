...
namespace Mageplaza\Vlibrary\Block\Post;
...
use Mageplaza\Vlibrary\Model\LikeFactory;
 ...
 */
class View extends \Mageplaza\Vlibrary\Block\Listpost
{
    /**
     * config logo vlibrary path
     */
    const LOGO = 'mageplaza/vlibrary/logo/';
     ...
     * @param \Mageplaza\Vlibrary\Model\CommentFactory $commentFactory
     * @param \Mageplaza\Vlibrary\Model\LikeFactory $likeFactory
    ...
    {
        return $this->helperData->getVlibraryUrl('post/rss');
    }
    ...
    {
        return $this->helperData->getVlibraryUrl($topic, Data::TYPE_TOPIC);
    }
    ...
    {
        return $this->helperData->getVlibraryUrl($tag, Data::TYPE_TAG);
    }
    ...
    {
        return $this->helperData->getVlibraryConfig('comment/' . $code);
    }
    ...
    public function getCommentsTree($comments, $cmtId)
    {
        ...
        foreach ($comments as $comment) {
            if ($comment['reply_id'] == $cmtId && $comment['status'] == 1) {
                ...
                $isLiked = ($this->isLiked($comment['comment_id'])) ? "mpvlibrary-liked" : "mpvlibrary-like";
            ...
            if ($catId = $this->getRequest()->getParam('cat')) {
                ...
                if ($category->getId()) {
                    $breadcrumbs->addCrumb($category->getUrlKey(), [
                        ...
                        'link'  => $this->helperData->getVlibraryUrl($category, Data::TYPE_CATEGORY)
                    ]);
                }
            }
    ...
    public function getVlibraryTitle($meta = false)
    {
        ...
        if ($meta) {
            if ($post->getMetaTitle()) {
                array_push($vlibraryTitle, $post->getMetaTitle());
            } else {
                array_push($vlibraryTitle, ucfirst($post->getName()));
            }
