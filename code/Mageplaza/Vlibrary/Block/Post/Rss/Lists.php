...
namespace Mageplaza\Vlibrary\Block\Post\Rss;
...
class Lists extends AbstractBlock implements DataProviderInterface
{
    ...
    {
        $this->setCacheKey('rss_vlibrary_posts_store_' . $this->getStoreId());
    ...
    public function getRssData()
    {
        ...
        $data = [
            'title'       => $title,
            'description' => $title,
            'link'        => $storeUrl . 'vlibrary/post/rss.xml',
            'charset'     => 'UTF-8',
    ...
    public function getFeeds()
    {
        $data = [];
        if ($this->isAllowed()) {
            $url = $this->rssUrlBuilder->getUrl(['type' => 'vlibrary_posts']);
