...
class RelatedPost extends Template
{
    ...
    public function getRelatedPostList()
    {
        if ($this->_relatedPosts == null) {
            ...
            $collection->getSelect()
                ->join(
                    [
                        'related' => $collection->getTable('mageplaza_vlibrary_post_product')
                    ],
    ...
    public function getLimitPosts()
    {
        if ($this->_limitPost == null) {
            $this->_limitPost = (int) $this->helperData->getVlibraryConfig('product_post/product_detail/post_limit') ?: 1;
        }
