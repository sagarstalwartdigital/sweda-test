...
class Pack extends \Magento\Framework\Model\AbstractModel implements PackInterface, IdentityInterface
{
     ...
     * @var null|array
     */
    private $productQtyAssociations = null;
     ...
     * @return int
     */
    public function getChildProductQty($productId)
    {
        if ($this->productQtyAssociations === null) {
            $this->productQtyAssociations = $this->serializer->unserialize(
                $this->getData(PackInterface::PRODUCTS_INFO)
            );
        }

        return (int) ($this->productQtyAssociations[$productId]['quantity'] ?? 1);
    }
}
