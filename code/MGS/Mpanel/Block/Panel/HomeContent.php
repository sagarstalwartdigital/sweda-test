...
class HomeContent extends AbstractPanel
{
	...
	public function getEditPanel() {
        ...
        $html .='<li class="up-link"><a title="' . __('Move Up') . '" onclick="return false;" href="#" class="moveuplink"><em class="fa fal fa-arrow-up">&nbsp;</em></a></li>';
        $html .='<li class="down-link"><a title="' . __('Move Down') . '" onclick="return false;" href="#" class="movedownlink"><em class="fa fal fa-arrow-down">&nbsp;</em></a></li>';
        $html .='<li><a id="section-'.$this->_section->getId().'-edit" href="' . $this->_urlBuilder->getUrl('mpanel/edit/section', ['id' => $this->_section->getId()]) . '" class="popup-link" title="' . __('Edit Section') . '" onclick="return false"><em class="fa fas fa-cogs"></em></a></li>';
        $html .='<li><a href="#" title="' . __('Delete Section') . '" onclick="if(confirm(\'' . __('Are you sure you would like to remove this section?') . '\')) removeSection(' . $this->_section->getId() . '); return false"><em class="fa fas fa-times"></em></a></li>';
        $html .='</ul></div>';
	...
	public function getBlockClass($section, $col, $arrClass, $key, $canUsePanel){
		...
		$colLarges = json_decode($section->getLargeCols(), true);
		if(is_array($colLarges) && isset($colLarges[$key])){
			$class .= ' col-lg-'.$colLarges[$key];
		}
		$colMedium = json_decode($section->getMediumCols(), true);
		if(is_array($colMedium) && isset($colMedium[$key])){
			$class .= ' col-md-'.$colMedium[$key];
		}
