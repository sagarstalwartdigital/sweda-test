...
class Section extends \Magento\Framework\App\Action\Action
{
    ...
    {
		if(($this->customerSession->getUsePanel() == 1) && ($id = $this->getRequest()->getParam('id'))){
			...
			$model = $this->_objectManager->create('MGS\Mpanel\Model\Section')->load($id);
			...
			if (!$model->getId()) {
				...
				if(isset($data['remove_background']) && ($data['remove_background']==1)){
					if($model->getBackgroundImage()!=''){
						$filePath = $this->_filesystem->getDirectoryRead(DirectoryList::MEDIA)->getAbsolutePath('mpanel/backgrounds') . $model->getBackgroundImage();
				...
				if(isset($_FILES['background_image']['name']) && $_FILES['background_image']['name'] != '') {
					...
					if(($file['name']!='') && ($file['size'] >0)){
						...
						$path = $this->_filesystem->getDirectoryRead(DirectoryList::MEDIA)->getAbsolutePath('mpanel/backgrounds');
						$uploader->save($path);
				...
				if (isset($data['large_cols']) && count($data['large_cols']) > 0) {
					$data['large_cols'] = json_encode($data['large_cols']);
				} else {
					$data['large_cols'] = NULL;
				}
				
				if (isset($data['medium_cols']) && count($data['medium_cols']) > 0) {
					$data['medium_cols'] = json_encode($data['medium_cols']);
				} else {
					$data['medium_cols'] = NULL;
				}
				...
				if ($model->getBlockCols() != '') {
                    ...
                    if ($newCols != '') {
                        ...
                        if ($lastKey > $lastNewKey) {
                            ...
                            if (count($arrKey) > 0) {
                                $childs = $this->_objectManager->create('MGS\Mpanel\Model\Childs')->getCollection()
