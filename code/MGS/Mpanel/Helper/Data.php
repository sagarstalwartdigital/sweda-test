...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
	...
	/**
	 * Retrieve current base url in base64 encoding
	 *
	 * @return string
	 */
			...
			if ($newLabel != '') {
				if (!(empty($newToDate))) {
					if (!(empty($newFromDate)) && ($newFromDate < $now) && ($newToDate > $now)) {
						//$html.='<span class="product-label123 new-label"><span>'.$newLabel.'</span></span>';
						$html .= '<div class="new sale"><a href="#">' . $newLabel . ' </a></div>';
						$numberLabel = 1;
					}
				}
			}
			...
			if ($this->getStoreConfig('mpanel/catalog/sale_label_discount') == 1) {
				...
				if (($finalPrice < $price) && ($saleLabel != '')) {
					if ($numberLabel == 1) {
						...
						$html .= '<div class="sale"><a href="#">' . $saleLabel . ' </a></div>';
					} else {
						$html .= '<div class="sale"><a href="#">' . $saleLabel . ' </a></div>';
					}
				}
	...
	public function getProductBackgroundSetting($storeId, $themeName)
	{
		$setting = [];
		$dir = $this->_filesystem->getDirectoryRead(DirectoryList::APP)->getAbsolutePath('code/MGS/Mpanel/data/themes/' . $themeName);
		$themeStyleFile = $dir . '/theme_style.xml';
		if (is_readable($themeStyleFile)) {
			$parsedArray = $this->_parser->load($themeStyleFile)->xmlToArray();
			if (isset($parsedArray['class_setting']['product_background'])) {
				foreach ($parsedArray['class_setting']['product_background'] as $classAttribute => $classString) {
					$classAttribute = str_replace('_', '-', $classAttribute);
					$setting[$classString] = [$classAttribute => $this->getStoreConfig('mpanel/product_details/background_color', $storeId)];
				}
			}
		}
		$setting = array_filter($setting);
	...
	public function getPanelUploadImages($type)
	{
		...
		if (is_dir($dir)) {
			if ($dh = opendir($dir)) {
				...
				foreach ($files as $file) {
					$file_parts = pathinfo($dir . $file);
					if (isset($file_parts['extension']) && in_array(strtolower($file_parts['extension']), ['jpg', 'jpeg', 'png', 'gif', 'mp4'])) {
						$result[] = $file;
					}
				}
			}
	...
	public function convertContent($layoutContent, $builderContent = NULL)
	{
		...
		if ($this->getStoreConfig('mgstheme/header/absolute_header')) {
			$layoutContent = str_replace('<header class="header">', '<div id="placeholder-header" class="absolute-header"></div><header class="header ' . $this->getHeaderClass() . '">', $layoutContent);
		} else {
			$layoutContent = str_replace('<header class="header">', '<div id="placeholder-header"></div><header class="header ' . $this->getHeaderClass() . '">', $layoutContent);
		}
	...
	public function generateCssByStore($storeId)
	{
		...
		if ($notFoundBackgroundImage != '') {
			$notFoundBgFolderName = \MGS\HebesTheme\Model\Config\Backend\BgNotFound::UPLOAD_DIR;
		...
		$fontStyle = [
			...
			'#mainMenu li a.level0, .navigation ul.container .level0 > a' => [
				'font-family' => str_replace('+', ' ', $this->getStoreConfig('mgstheme/fonts/menu_level0', $storeId)),
			],
			'#mainMenu' => [
		...
		if (($this->getStoreConfig('mpanel/product_details/background_color', $storeId) != '') && ($this->getStoreConfig('mpanel/product_details/background_color', $storeId) != 'transparent')) {
			$productColorSetting = $this->getProductBackgroundSetting($storeId, $themeName);
			if (count($productColorSetting) > 0) {
				foreach ($productColorSetting as $class => $style) {
					$style = array_filter($style);
					if (count($style) > 0) {
						$html .= $class . '{';
						foreach ($style as $_style => $value) {
	...
	public function checkWishlist($product)
	{
		foreach ($this->_wishlistHelper->getWishlistItemCollection() as $_wishlist_item) {
	...
	// get Wysiwyg media url
	public function getWysiwygMediaUrl($type, $image)
	{
		$mediaurl = $this->_storeManager->getStore()->getBaseUrl(\Magento\Framework\UrlInterface::URL_TYPE_MEDIA);
		$path = "wysiwyg/" . $type . "/" . $image;
		$result = $mediaurl . $path;
		return $result;
	}

	public function getSwedaFormMediaUrl()
	{
		$mediaurl = $this->_storeManager->getStore()->getBaseUrl(\Magento\Framework\UrlInterface::URL_TYPE_MEDIA);
		$path = "catalog/product/file/sweda_forms/";
		$result = $mediaurl . $path;
		return $result;
	}


	public function getLink($text,$link){
