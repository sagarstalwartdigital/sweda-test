        ...
        $.widget('mage.SwatchRenderer', widget, {
			...
			_init: function () {
				...
				if (this.options.jsonConfig !== '' && this.options.jsonSwatchConfig !== '') {
					...
					var inviElement = this.element;
					if($(this.element).find("div.swatch-attribute-options .swatch-option").length)
					{
						$(this.element).find("div.swatch-attribute-options .swatch-option").click(function(inviEvent){
							if (inviEvent.originalEvent !== undefined) {
		                    	var selectedColorElmenetId = $(this).attr('id');
		                    	$(inviElement).find("select.swatch-select-customcolor").val(selectedColorElmenetId).change();
		                    }
		                });
					}
				} else {
			...
			updateBaseImageHorizontal: function(imagesToUpdate) {
				...
				$(".product.media").html(img_change);
				var inviActivePosition = $(".product.media #owl-carousel-gallery .item-image").index($(".invi-is-main"));
				$('#owl-carousel-gallery').owlCarousel({
					items: 1,
					autoplay: false,
					lazyLoad: false,
				...
				if(inviActivePosition)
					$("#owl-carousel-gallery").trigger("to.owl.carousel", [inviActivePosition, 1]);
