...
class Category extends AbstractDb
{
    ...
    {
        $this->_init('mgs_gallery_category', 'category_id');
    }
    ...
    protected function _beforeDelete(AbstractModel $object)
    {
        ...
        $this->getConnection()->delete($this->getTable('mgs_gallery_category_store'), $condition);
        $this->getConnection()->delete($this->getTable('mgs_gallery_category_post'), $condition);
    ...
    protected function _afterSave(AbstractModel $object)
    {
        ...
        $table = $this->getTable('mgs_gallery_category_store');
    ...
    protected function _getLoadSelect($field, $value, $object)
    {
        ...
        if ($object->getStoreId()) {
            ...
            $select->join(
                ['mgs_gallery_category_store' => $this->getTable('mgs_gallery_category_store')],
                $this->getMainTable() . '.category_id = mgs_gallery_category_store.category_id',
                []
            )->where(
                'status = ?',
                1
            )->where(
                'mgs_gallery_category_store.store_id IN (?)',
                $storeIds
            )->order(
                'mgs_gallery_category_store.store_id DESC'
            )->limit(
                1
            );
        }
    ...
    protected function _getLoadByIdentifierSelect($identifier, $store, $isActive = null)
    {
        $select = $this->getConnection()->select()->from(
            ...
            ['cps' => $this->getTable('mgs_gallery_category_store')],
    ...
    public function lookupStoreIds($categoryId)
    {
        ...
        $select = $connection->select()->from(
            $this->getTable('mgs_gallery_category_store'),
            'store_id'
        )->where(
            'category_id = ?',
            (int)$categoryId
    ...
    public function checkUrlKeyExits(AbstractModel $object)
    {
        ...
        $connection = $this->getConnection();
        $select = $connection->select()->from(
            $this->getTable('mgs_gallery_category'),
            'category_id'
        )
            ->where(
                'url_key = ?',
