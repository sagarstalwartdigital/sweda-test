...
namespace MGS\Gallery\Model\Resource;
...
class Post extends AbstractDb
{
    ...
    {
        $this->_init('mgs_gallery_post', 'post_id');
    }

    protected function _beforeDelete(AbstractModel $object)
    {
        $condition = ['post_id = ?' => (int)$object->getId()];
        $this->getConnection()->delete($this->getTable('mgs_gallery_comment'), $condition);
        $this->getConnection()->delete($this->getTable('mgs_gallery_post_store'), $condition);
    ...
    protected function _afterSave(AbstractModel $object)
    {
        ...
        $table = $this->getTable('mgs_gallery_category_post');
        ...
        $table = $this->getTable('mgs_gallery_post_store');
    ...
    protected function _getLoadSelect($field, $value, $object)
    {
        ...
        if ($object->getStoreId()) {
            ...
            $select->join(
                ['mgs_gallery_post_store' => $this->getTable('mgs_gallery_post_store')],
                $this->getMainTable() . '.post_id = mgs_gallery_post_store.post_id',
                []
            )->where(
                'status = ?',
                1
            )->where(
                'mgs_gallery_post_store.store_id IN (?)',
                $storeIds
            )->order(
                'mgs_gallery_post_store.store_id DESC'
            )->limit(
                1
            );
        }
    ...
    protected function _getLoadByIdentifierSelect($identifier, $store, $isActive = null)
    {
        $select = $this->getConnection()->select()->from(
            ...
            ['cps' => $this->getTable('mgs_gallery_post_store')],
    ...
    public function lookupStoreIds($postId)
    {
        ...
        $select = $connection->select()->from(
            $this->getTable('mgs_gallery_post_store'),
            'store_id'
        )->where(
            'post_id = ?',
            (int)$postId
    ...
    public function lookupCategories($postId)
    {
        $connection = $this->getConnection();
        $select = $connection->select()->from(
            $this->getTable('mgs_gallery_category_post'),
            'category_id'
        )->where(
            'post_id = ?',
            (int)$postId
    ...
    public function checkUrlKeyExits(AbstractModel $object)
    {
        ...
        $connection = $this->getConnection();
        $select = $connection->select()->from(
            $this->getTable('mgs_gallery_post'),
            'post_id'
        )
            ->where(
                'url_key = ?',
