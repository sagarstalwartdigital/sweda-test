...
class Save extends Gallery
{
    public function execute()
    {
        if ($this->getRequest()->getPostValue()) {
            try {
                $galleryHelper = $this->_objectManager->create('MGS\Gallery\Helper\Data');
                ...
                if (isset($_FILES['thumbnail']['name']) && $_FILES['thumbnail']['name'] != '') {
                    ...
                    $uploader->setAllowedExtensions(['jpg', 'jpeg', 'gif', 'png', 'svg' ,'mp4', 'pdf', 'doc']);
                    ...
                    if ($uploader->checkAllowedExtension($ext)) {
                        $path = $this->_objectManager->get('Magento\Framework\Filesystem')->getDirectoryRead(DirectoryList::MEDIA)
                            ->getAbsolutePath('mgs_gallery/');
                        $uploader->save($path);
                        $fileName = $uploader->getUploadedFileName();
                        if ($fileName) {
                            $data['post']['thumbnail'] = 'mgs_gallery' . $fileName;
                        }
                    } else {
                ...
                if (isset($_FILES['image']['name']) && $_FILES['image']['name'] != '') {
                    ...
                    if ($uploader->checkAllowedExtension($ext)) {
                        ...
                        $fileName = $uploader->getUploadedFileName();
                        if ($fileName) {
                            $data['post']['image'] = 'mgs_gallery' . $fileName;
                        }
                    } else {
                        $this->messageManager->addError(__('Disallowed file type.'));
                        return $this->redirectToEdit($model, $data);
                ...
                if (!isset($data['post']['url_key']) || $data['post']['url_key'] == '') {
                    ...
                    $existUrlKeys = $this->_objectManager->create('MGS\Gallery\Model\Post')->getCollection()->addFieldToFilter('url_key', ['eq' => $urlKey]);
                ...
                if (isset($data['post']['tags'])) {
                    ...
                    foreach ($tags as $key => $tag) {
                        $tags[$key] = $galleryHelper->convertSlashes($tag, 'forward');
                    }
               ...
               $userData = $this->_objectManager->get('Magento\Backend\Model\Auth\Session')->getUser()->getData();
                ...
                if ($this->getRequest()->getParam('back')) {
                    $this->_redirect('gallery/post/edit', ['post_id' => $model->getId()]);
                    return;
                }
                ...
                if (!empty($id)) {
                    $this->_redirect('gallery/post/edit', ['post_id' => $id]);
                } else {
                    $this->_redirect('gallery/post/new');
                }
                return;
            } catch (\Exception $e) {
                ...
                $this->_redirect('gallery/post/edit', ['post_id' => $this->getRequest()->getParam('post_id')]);
                return;
            }
        }
        $this->_redirect('gallery/post/index');
