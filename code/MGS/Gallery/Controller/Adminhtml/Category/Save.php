        ...
        if ($this->getRequest()->getPostValue()) {
            try {
                ...
                if (!isset($data['category']['url_key']) || $data['category']['url_key'] == '') {
                    ...
                    $existUrlKeys = $this->_objectManager->create('MGS\Gallery\Model\Category')->getCollection()->addFieldToFilter('url_key', ['eq' => $urlKey]);
                ...
                if ($this->getRequest()->getParam('back')) {
                    $this->_redirect('gallery/category/edit', ['category_id' => $model->getId()]);
                    return;
                }
                ...
                if (!empty($id)) {
                    $this->_redirect('gallery/category/edit', ['category_id' => $id]);
                } else {
                    $this->_redirect('gallery/category/new');
                }
                return;
            } catch (\Exception $e) {
                ...
                $this->_redirect('gallery/category/edit', ['category_id' => $this->getRequest()->getParam('category_id')]);
                return;
            }
        }
        $this->_redirect('gallery/category/index');
