...
class Category extends \Magento\Backend\Block\Widget\Form\Generic implements \Magento\Backend\Block\Widget\Tab\TabInterface
{
	...
	public function getCategory(){
		...
		$category = $this->getModel('Magento\Catalog\Model\Category')->setStoreId($this->getRequest()->getParam('store'))->load($categoryId);
		return $category;
	}
	...
	public function getTreeCategory($category, $parent, $ids = array()){
		...
		$html[] = '/><label for="radio'.$category->getId().'">' . __($category->getName()) . '</label>';
		...
		if($childrenCount>0){
			foreach ($children as $child) {
				$_child = $this->getModel('Magento\Catalog\Model\Category')->setStoreId($this->getRequest()->getParam('store'))->load($child->getId());
